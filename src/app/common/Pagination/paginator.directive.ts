...
// import { link } from "fs";
  ...
  export class PaginatorDirective {
          ...
          for (let i = 0; i < nodeArray.length; i++) {
            if (nodeArray[i].nodeName === "BUTTON") {
              if (nodeArray[i].innerHTML.length > 100 && nodeArray[i].disabled) {
                nodeArray[i].classList.remove('mat-paginator-navigation-previous', 'mat-icon-button');
                nodeArray[i].removeAttribute('style');
                nodeArray[i].classList.add('arrow_button-paginator'); 
              } else if (
    ...
    private createButton(i: any, pageIndex: number): any {
      ...
      this.ren.addClass(linkBtn, "Number_button");
      const pagingTxt = isNaN(i) ? this._pageGapTxt : +(i + 1);
      const text = this.ren.createText(pagingTxt + "");
      switch (i) {
        case pageIndex:
          this.ren.setAttribute(linkBtn, "disabled", "disabled");
          this.ren.addClass(linkBtn,'activePage');
          break;
